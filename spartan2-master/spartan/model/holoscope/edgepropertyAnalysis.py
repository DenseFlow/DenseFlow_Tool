...
class MultiEedgePropBiGraph:
    def __init__(self, wadjm):
        ...
        self.nU , self.nV = wadjm.shape #UVsize
        self.indegrees = self.wadjm.sum(0).getA1() #入度
    ...
    def load_from_edgeproperty(self, profnm, mtype=coo_matrix,
        ...
        with myreadfile(profnm, 'rb') as fin:
            ...
            for line in fin:
                ...
                x.append(u)
                y.append(m)
                data.append(idx) #data store the index of edge properties data存储的是边特征的序号，
    ...
    def trans_array_to_edgeproperty(self, arr:STensor, mtype=coo_matrix, dtype=int):
        ...
        for i in range(len(freqs)):
            ...
            if (u,v) not in uvdict:
                ...
                eprop.append([]) #如果当前两个节点之间没有交易，边特征加入空
    ...
    def setup_rate4all_sinks(self):
        ...
        for i in range(len(self.eprop)):
            ...
            if min(propvals)<1:
                self.eprop[i] = np.digitize(self.eprop[i], bins=[-1,25,50,75,100])
            else:
                self.eprop[i] = np.digitize(self.eprop[i], bins=[-1,25,50,75,100])
        ee = self.eprop.flatten()
        #print(ee)
    ...
    def setupsuspects(self, users):
        ...
        cols = set(cols) & self.inbdcolset #cols是列合集
        spv = {}
        for col in cols:
            spids = suspmlt.data[col] #data是特征序号，索引当前列特征序号spids
            #print('spv:',spids)
            #property indices of suspect sink
            #only consider those objects have more than inbd edges with suspusers
            sumts = np.concatenate(self.eprop[spids]) #特征拼接在一起
            #sumts= np.bincount(sumts, minlength=3) #加了一行
            spv[col]=sumts #spv是一个字典，可以根据col查询得到边特征
        self.spv = spv
        return
    ...
    def suspratedivergence(self, neutral=False, delta=False):
        ...
        for col in cols:
            ...
            rs = self.spv[col]#剩余的
            #print('rs',rs)
            shis=np.bincount(rs, minlength=6)
            ahis = self. ahists[col]
            #print('ahis',ahis)
            ahis = np.bincount(ahis, minlength=6)
            #print(self.spv,self.ahists)
            ohis=ahis-shis
            #spv和ahists出现维度不对的问题？？？
